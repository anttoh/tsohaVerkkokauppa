...
def hide(listing_id):
    sql = """UPDATE listings SET visible=0 
             WHERE listing_id=:listing_id"""
    db.session.execute(
...
def set_visible(listing_id):
    sql = """UPDATE listings SET visible=1 
             WHERE listing_id=:listing_id"""
    db.session.execute(
        sql, {"listing_id": listing_id})
    db.session.commit()
...
def get_list(item_id):
    sql = """SELECT listings.price, users.username, listings.description, listings.listing_id 
             FROM listings 
...
def unsold_items():
    sql = """SELECT listings.listing_id, items.name, makers.name, listings.price 
             FROM listings 
             INNER JOIN items ON listings.item_id=items.item_id 
...
def create(item_name, maker_name, price, description, tags_list):
    ...
    try:
        ...
        sql = """INSERT INTO listings (item_id, seller_id, price, description, visible) 
                 VALUES (:item_id, :seller_id, :price, :description, :visible) 
