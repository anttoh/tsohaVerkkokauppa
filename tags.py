...
def create_listing_tag(listing_id, tag_id):
    sql = "INSERT INTO listing_tag (listing_id,tag_id) VALUES (:listing_id,:tag_id)"
    db.session.execute(sql, {"listing_id": listing_id, "tag_id": tag_id})
    db.session.commit()


def get_list(listing_id):
    sql = "SELECT tags.name FROM tags INNER JOIN listing_tag ON tags.tag_id=listing_tag.tag_id WHERE listing_tag.listing_id=:listing_id"
